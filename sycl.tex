...
Eine wichtige Abstraktion sind die \textit{compute units} (CU). Diese lassen
sich auf einen oder mehrere Teile des Beschleunigers abbilden und sind in der
Lage, einen Kernel auszuführen. Bei einer CPU lässt sich eine CU also auf einen
...
keine Obergrenze für die CU"=Anzahl vorgibt.
...
Die SIMD"=Fähigkeiten der CUs werden ebenfalls abstrahiert und als
\textit{work"=item} bezeichnet. Eine \textit{work"=group} besteht aus mindestens
einem \textit{work"=item}, wobei die Implementierung auch eine maximale Anzahl
von \textit{work"=items} festlegen kann. \textit{Work"=items} werden zueinander
asynchron ausgeführt, lassen sich jedoch über Funktionen der gemeinsamen
\textit{work"=group} synchronisieren. Es ist jedoch nicht möglich,
\textit{work"=items} verschiedener \textit{work"=groups} zu synchronisieren.

Durch diese Hardware"=Abstraktion wird aus Sicht der Programmierers ein
Indexraum aufgespannt, in dem jedem \textit{work"=item} ein eindeutiger Index
innerhalb der \textit{work"=group} sowie der Menge aller \textit{work"=items}
zugewiesen wird. Diese Indizes werden als lokale bzw. globale Indizes
bezeichnet.

Diese Form der Hardware"=Abstraktion erlaubt dem Programmierer auch die
einfache Implementierung mehrdimensionaler Probleme, da sowohl die
\textit{work"=groups} als auch die \textit{work"=items} in bis zu drei
Dimensionen angeordnet werden können.
...
automatisch erkannt und die Kernel in der richtigen Reihenfolge ausgeführt. In
diesem Aspekt unterscheidet sich SYCL von \gls{opencl}, das neben vollständig
seriellen Warteschlangen nur asynchrone Warteschlangen kennt, bei denen die
richtige Sortierung der Kernel Aufgabe des Programmierers ist.
