...
auf einer GPU die Multiprozessoren und SIMD"=Einheiten. Durch
\texttt{parallel\_for} kann auf einem FPGA ebenfalls eine SIMD"=Schaltung
...
Gesamtzahl der \textit{work"=items} muss immer angegeben werden. Sofern der
Programmierer darauf verzichtet, die Größe der \textit{work"=groups} selbst
festzulegen, ist es Aufgabe der SYCL"=Implementierung, die Zahl der nötigen
\textit{work"=groups} zu bestimmen und die Zuordnung der \textit{work"=items}
durchzuführen. Mit dieser Entscheidung verliert der Programmierer jedoch die
Möglichkeit, gruppenweite Operationen durchzuführen, wie z.B. die
Synchronisierung innerhalb der \textit{work"=group}.
...
verarbeitet werden können (siehe den
Anhang~\ref{anhang:source:cpp:syclexceptions}).
...
werden (siehe den Anhang~\ref{anhang:source:cpp:syclprofiling}).
