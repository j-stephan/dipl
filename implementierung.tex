...
abbildet.
% Was wären denn grob für Designänderungen in Alpaka nötig, um die Frage der runtime-Plattform- und Device-Auswahl zu lösen?
% Es gibt ja endlich viele generische Devices (CPU, GPU, FPGA, Accelerator)
% Würde das denn für compile-Zeit ausreichen? Die Herstellerspezialisierung kann ja dann zur Runtime/treiberseitig erfolgen.
...
% Stichwort flushing und profiling
...
implementiert.
% letzter Satz etwas verwirrend, was fehlt denn noch, was oben noch nicht beschrieben ist?
% Ansonsten weglassen
...
% ist mit Kernel-Task von nun an immer Alpaka-Kernel-Task gemeint?
...
% nützlich wäre ein Abschnitt in Grundlagen zu den Parallelitätsebenen, auf diese
% sich später bezogen werden können. "Keinerlei Parallelität" ist dann entsprechend zu konkretisieren.
% (u.a. gibt es bit-/word-/instruction-/thread-/program-level parallelisms)
% (ggf. auch für [Alpaka] memory level (global, static|dynamic shared, etc))
...
Da Alpaka diese Informationen in eigenen Datenstrukturen mit einem
anderen Format abbildet (Anzahl der \textit{Blocks} und Anzahl der \textit{Threads} pro
\textit{Block}), ist eine Konvertierung
...
entspricht) und führt die Konvertierung in ein SYCL"=kompatibles Format durch. Die
...
% was ist der Fehler in der Spezifikation, wenn
% doch nur die Implementierungen für die Zeiger fehlten
...
\end{code}
% ist C++14 (return type deduction), für Prototyp und SYCL Compiler (C++17) ok (sonst ist C++11 erforderlich)
% daher: Code nur für SYCL Compiler sichtbar?
...
% auch rsqrt und sincos? Wie sieht es mit float/double precision-spezifischen Überladungen aus?
...
\end{figure}
% Grafik ggf. geeignet, um Status der Implementierungen hervorzuheben
% was beinhaltet überwiegend workarounds,
% was beinhaltet plattformspezifische Inkompatibilitäten,
% welche Implementierung bricht das Alpaka-Interface (bspw. EventSycl),
% was ist nicht implementiert (zB random numbers; Zeit; statische global|constant Variablen),
% Ansonsten eigene Übersicht dazu wünschenswert