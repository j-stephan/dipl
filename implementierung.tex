...
starten, was der Organisation der Abhängigkeiten zwischen \textit{Kerneln} sehr
dienlich ist.
...
Parametern aufruft. SYCL verfügt über drei verschiedene Möglichkeiten für den
Kernel"=Start (siehe Abschnitt~\ref{sycl:konzepte:kernelstart}):
...
\begin{itemize}
    ...
    \item \texttt{parallel\_for\_work\_group} bietet dieselbe Funktionalität wie
          ...
          Schwerpunkt. Mit diesem Ansatz wird der Kernel in zwei separate
          Bereiche aufgeteilt: Einmal auf Ebene der \textit{work"=groups} und
          einmal auf Ebene der\textit{work"=items}, wobei die letztere der
          ersteren untergeordnet ist bzw. aus ihr heraus aufgerufen wird.
          Befehle auf der \textit{work"=group}"=Ebene werden genau einmal für
          die gesamte \textit{work"=group} ausgeführt, Befehle auf der Ebene der
          \textit{work"=items} genau einmal pro \textit{work"=item}. Dadurch
          hat der Programmierer die Möglichkeit, seine Algorithmen hierarchisch
          zu gliedern. Dieses Konzept findet sich nicht in Alpakas
          Work"=Division"=Konzept, weshalb in dieser Implementierung
          \texttt{parallel\_for} als Kernel"=Start"=Funktion gewählt wurde.
\end{itemize}
...
sich von anderen SYCL"=\textit{Kerneln} unterscheiden und global sichtbar sein
muss. Daher wurde für das SYCL"=Backend die Klasse \texttt{sycl::detail::kernel}
eingeführt, welche den Typ der Alpaka"=\textit{Kernel}"=Funktion
(\texttt{TKernelFnObj}) als Template"=Parameter entgegennimmt.

Im Quelltext~\ref{implementierung:task:kernel:launch:src} wird der Vorgang des
...
hinaus findet er nur auf Puffer des Typs \texttt{uint8\_t} Anwendung, da es sich
hier um eine byte"=weise Operation handelt. Puffer anderer Typen werden deshalb
...
seinem Kernel nutzen kann. Diese Idee bedarf jedoch einer vorherigen Diskussion
