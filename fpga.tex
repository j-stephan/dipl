...
erläutert im Anschluss den Aufbau moderner \gls{fpga}"=Architekturen sowie
...
werden (engl. \textit{full-custom ASIC}), lässt sich so -- unter Inkaufnahme
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption[abstrakter FPGA-Aufbau]
            {abstrakter FPGA-Aufbau \cite[nach][10-14]{hawkins2010}}
...
\begin{itemize}
    ...
    \item Eingabe/Ausgabe-Blöcke (engl. \gls{iob}) werden zur Steuerung des
          ...
          benutzt. Es gibt mehrere \gls{iob}"=Typen in der
          UltraScale+\-Ar\-chi\-tek\-tur, die z.B. verschiedene I/O"=Standards
          oder uni- und bidirektionale Kommunikation unterstützen. \cite[vgl.
          die ausführliche I/O"=Beschreibung in][Kapitel 1 und 2]{ultrascaleio2019}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \draw (0.0, 5) rectangle (7.5, 7.5)
            ...
            node [pos = 0.1] {SLR2};
    \end{tikzpicture}
    \caption[Aufbau eines XCU200-FPGAs]
            {Aufbau eines XCU200-FPGAs \cite[nach][5]{alveo2019}}
...
\begin{table}[htb]
    ...
    \caption[Ressourcen der dynamischen Regionen eines XCU200-FPGAs]
            {Ressourcen der dynamischen Regionen eines XCU200-FPGAs
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \draw [fill = HKS92!90] (0.0, 0.0) rectangle (1.0, 8.0)
            node [pos = 0.5, rotate = 90, color = white] {externe I/O};
        ...
        \draw [fill = HKS92!70] (2.5, 0.0) rectangle (3.5, 8.0)
            node [pos = 0.5, rotate = 90, color = white] {I/O, Takt, Speicherschnittstelle};
    ...
    \caption[spaltenweise Verteilung der FPGA-Ressourcen]
            {spaltenweise Verteilung der FPGA-Ressourcen
...
\begin{figure}[htb]
    ...
    \caption[Aufteilung der FPGA-Ressourcen auf \textit{clock regions}]
            {Aufteilung der FPGA-Ressourcen auf \textit{clock regions}
             \cite[nach][22]{ultrascale2019}}
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption[Y-Diagramm nach Gajski]
            {Y-Diagramm nach Gajski \cite[nach][10]{kesel2013}}
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption[Modell eines endlichen Automaten]
            {Modell eines endlichen Automaten (Moore"=Schaltwerk)
...
END reg2 ;
    \end{minted}
    \caption[Entity eines 2-Bit-Registers]
            {\textit{Entity} eines 2-Bit-Registers \cite[siehe][26]{kesel2013}}
...
END beh;
    \end{minted}
    \caption[Verhaltensbeschreibung eines 2-Bit-Registers]
            {Verhaltensbeschreibung eines 2-Bit-Registers \cite[siehe][28]{kesel2013}}
...
END struct;
    \end{minted}
    \caption[Strukturbeschreibung eines 2-Bit-Registers]
            {Strukturbeschreibung eines 2-Bit-Registers \cite[siehe][36]{kesel2013}}
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption[FPGA-Implementierung einer mehrstufigen Berechnung]
            {FPGA-Implementierung einer mehrstufigen Berechnung
             \cite[nach][21]{hlsintro2019}}
...
weiterer Taktzyklus aufgefasst werden. Die Berechnung eines einzelnen
Ergebnisses benötigt dadurch drei zusätzliche Taktzyklen. Durch die eingefügten
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption[FPGA-Pipeline-Architektur]
            {FPGA-Pipeline-Architektur \cite[nach][22]{hlsintro2019}}
