...
von \textit{Task}"=Graphen, siehe
Abschnitt~\ref{alpaka:konzepte:abhaengigkeiten}), Alpaka übernimmt auch direkt
CUDA"=Begriffe (z.B. \textit{Grid}, \textit{Block} oder \textit{Thread}, siehe 
Abschnitt~\ref{alpaka:konzepte:abstraktion}). Im Hintergrund wird Alpaka auf
hersteller- oder hardware"=spezifische Schnittstellen, wie CUDA oder OpenMP
...
erkennt, existiert kein \texttt{Acc}"=Parameter für eine Alpaka"=\textit{Queue}
...
die die reinen Zeiger kapseln. Im Gegensatz zu SYCL sind die Puffer auf dem
\textit{Host} und \textit{Device} jedoch voneinander unabhängig. Es ist also
nicht möglich, einen gemeinsamen Puffer zu erzeugen, der die notwendigen Kopien
für den Programmierer unsichtbar im Hintergrund durchführt, wie es in
...
Innerhalb des \textit{Kernels} wird ein Index"=Raum aufgespannt (eine genaue
...
/* ab hier Überprüfung der Ergebnisse möglich */
    \end{minted}
    \caption{Synchronisation zwischen \textit{Host} und \textit{Device} in
             Alpaka}
...
Multiprozessoren, wodurch der \textit{Kernel} selbst skalierbar wird.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption[Abstraktionshierarchie aus Grid, Blocks, Warps und Threads]
            {Links: Abstraktionshierarchie mit einem \textit{Grid}, das aus
...
ein \textit{Element} -- also ein Datum -- bearbeiten zu lassen, ist nicht immer
der optimale Weg. So kann es beispielsweise sinnvoll sein, auf
CPU"=Beschleunigern mehrere \textit{Elements} pro \textit{Thread} innerhalb
kurzer Schleifen zu verarbeiten. Dadurch können gute Compiler selbstständig von
Vektorregistern der CPU Gebrauch machen. Alternativ kann der Programmierer dies
auch über alpaka"=fremde befehlssatzspezifische Compiler"=Intrinsiken explizit
selbst tun, gibt dadurch aber die Portabilität auf.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption[Struktureller Aufbau der Alpaka-Bibliothek]
            {Struktureller Aufbau der Alpaka-Bibliothek
             \cite[nach][40]{worpitz2015}}
